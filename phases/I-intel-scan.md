...
> 在已存在的项目（brownfield）里第一次用 Gantry 时，**先跑这个**。
...
Gantry 需要一份 CONTEXT.md 给 AI 用。请选择：
...
- 探测命中：<列出找到的文档>
- 用户选择：<选项编号 + 简述>
- 决策：<生成 CONTEXT.md / 以现有文档为准 / 跳过入场扫描>
...
- 跑完 → 用户继续原本意图（如 `docs/GO.md` + 一句新需求）
