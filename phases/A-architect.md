...
> **触发方式**：`/gantry-context architect` / `docs/GO.md` + `建立架构 / 架构梳理 / 重构架构 / architect / 重审 ADR`
> 不属于任何 change，不写 `PROPOSAL.md` / `CHANGE.md`。直接产出 / 重构 `.gantry/specs/ARCHITECTURE.md`。
...
- **ADR 重审**：`/gantry-context evolve` 多次提示"ADR 冲突 / 累积 ≥ 5"时升级处理
- **接手陌生项目**：跑完 `/gantry-context scan` 之后，想把架构理解结构化
...
- `@.gantry/specs/CONTEXT.md`（必读 · 技术栈 + 既有抽象 + 禁动清单）
- `@.gantry/specs/ARCHITECTURE.md`（如有，则是重构跑；如无，则是首跑）
- `@.gantry/specs/CHANGELOG.md`（看历史 change 演进轨迹）
- `@.gantry/specs/_archive/*/DESIGN.md`（按需 · 重构跑时抽样几份）
- 仓库代码（必读 · `src/` 顶层结构 + `package.json` / 等价依赖文件）
- `@gantry/templates/ARCHITECTURE.md`（结构模板）
...
读 `.gantry/specs/ARCHITECTURE.md`：
...
**首跑**：用 `write_to_file` 创建 `.gantry/specs/ARCHITECTURE.md`，按模板 8 段全填（不适用的段可留 N/A 但**不能删段**——后续 A-evolve 要按段 append）。

**重构**：

- 先备份：`cp .gantry/specs/ARCHITECTURE.md .gantry/specs/ARCHITECTURE.md.bak-<YYYY-MM-DD>`
...
- `.gantry/specs/ARCHITECTURE.md`（必产）
- `.gantry/specs/ARCHITECTURE.md.bak-<date>`（重构跑必产）
- 0~1 次对 `.gantry/specs/CONTEXT.md` 的同步 patch（仅当步骤 7.3 用户选 1）
- 在 `STATE.md` 写 `last_architect_at: <YYYY-MM-DD>`

## 约束

- **不动业务代码**：本工作流仅写 `.gantry/specs/` 内文件，禁动 `src/`
