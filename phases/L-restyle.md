...
- 在 frontmatter 加：
  ```yaml
  version: 2
  supersedes: _archive/<old-change-id>/UI-DESIGN.md
...
- 用户确认 → `phases/3-task.md`（按上述波次落实任务）
- → `phases/4-dev.md`（每任务 fresh context，UI 任务自动扫 anti-patterns）
- → `phases/6-review.md`（**第三轮 UI 视觉审查必跑**，含 v1 vs v2 对比段）
- → `phases/7-integration.md`（UAT 重点：老用户能否完成关键流程；视觉是否传达声明的调性）
