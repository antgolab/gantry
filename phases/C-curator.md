...
打开 `.gantry/specs/metrics/<YYYY-MM>.md`，重点看：
...
走到 `.gantry/specs/LESSONS.md` 末尾 `## 待审` 段。INTEGRATION 阶段提名的条目都在这。
...
遍历 `.gantry/specs/knowledge/*.md`：

#### A. 状态升级

条目默认 `status: draft`。符合以下条件之一 → 升 `reviewed`：

- 有人引用过（grep `.gantry/specs/` 和 git history）
...
```bash
find .gantry/specs/knowledge -name '*.md' -mtime +180 -exec grep -l 'status: reviewed' {} +
```
...
```bash
git add .gantry/specs/metrics/<YYYY-MM>.md .gantry/specs/LESSONS.md .gantry/specs/knowledge/
...
对比 `.gantry/specs/CONVENTIONS.md` 与实际代码约定：
...
> 以上阈值是经验默认值。建议运行 3 个月后用真实分布修正，在 `.gantry/specs/CONVENTIONS.md` 的 §11 记录项目实测阈值。
...
1. 写 `.gantry/specs/curator-handover-<YYYY-MM-DD>.md`：
...
- `.gantry/specs/metrics/<YYYY-MM>.md`（月度 metrics 报告）
- `.gantry/specs/LESSONS.md`（状态更新）
- `.gantry/specs/knowledge/*.md`（状态更新）
- `.gantry/specs/curator-handover-<date>.md`（交接时）
