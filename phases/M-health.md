...
> **触发方式**：`docs/GO.md` + `健康检查 / health / 体检 / 技术债扫描 / 巡检`
> 不属于任何 change，不写 `PROPOSAL.md` / `SPEC.md`（兼容期接受旧命名）。直接产出健康报告。
...
- 仓库根（不限单一 change）
- `@.gantry/specs/CONTEXT.md`（含技术栈 + 团队偏好）
- `@.gantry/specs/LESSONS.md`（已记录的失败教训）
- 最近 N 次的 `_archive/<change-id>/` 归档（看历史趋势）
- 当前 git log 最近 30 天
...
把工具输出**原样**贴入 `.gantry/specs/health/<YYYY-MM-DD>-HEALTH.md`。
...
读 `.gantry/specs/CONTEXT.md`「技术栈」段 + 扫清单文件定主语言：
...
1. **优先 `jscpd`**（全语言 · 5 分钟出结果）：`npx jscpd src/ --min-lines 5 --min-tokens 50 --reporters json --output .gantry/specs/health/tmp/`
...
| 分类 | 标准 |
|---|---|
| 🔴 **Critical** | ≥ 20 行重复块出现在 ≥ 3 处 · 被跨模块调用的公共导出无引用 · 死分支导致业务规则静默失效（参考 `examples/brooks-lint-paths.md` R6 案例）|
...
随机抽 5 个最近改动频繁的模块（用 `git log --pretty=format: --name-only | sort | uniq -c | sort -rn | head -5`），每个按 6 维（R1~R6，见 `phases/6-review.md` 第 2.1 节）打分。

#### 3.2 测试 6 维抽样

随机抽 5 个测试文件，按 T1~T6（见 `phases/5-test.md` 第 1.4 节）打分。
...
写入 `.gantry/specs/health/<YYYY-MM-DD>-HEALTH.md`，结构：
...
- 🔴 Critical → 自动开新 `PROPOSAL`（编号如 `health-fix-2026-q1`，兼容期接受旧 `CHANGE`），AI 进入 `0-change` 流程产 `PROPOSAL.md`
- 🟡 Scheduled → 追加到 `.gantry/specs/CONTEXT.md` 的「技术债」段
- 🟢 Monitored → 追加到 `.gantry/specs/LESSONS.md`（用 `观察` 类型 entry）
- 综合分趋势 → 长期保留在 `.gantry/specs/health/` 目录，可画图
- **冗余巡检特殊处理**：
  - 🔴 未用导出 / 死分支 → 合并到 health-fix CHANGE（不单开）
  - 🟡 重复块 → 记 `.gantry/specs/CONTEXT.md`「技术债」段，标"待抽公共函数"
  - 🟡 未用依赖 → **写到 CONTEXT.md「禁动清单」段**（标"下次清理窗口移除"），避免 AI 再次为已被移除的库写代码
  - 🟢 关联 `A-evolve` 下次同步时**主动提醒**：有 M 条没用的导出在 CONTEXT.md「既有抽象索引」里还列着，建议清理

## 输出

- `.gantry/specs/health/<YYYY-MM-DD>-HEALTH.md`（必产）
...
- [ ] 报告归入 `.gantry/specs/health/` 目录

## 触发下一步

- 有 🔴 Critical → 用户选「现在修」 → `phases/0-change.md` 开 health-fix change
