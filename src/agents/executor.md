...
capabilities:
  ...
  git: true
constraints:
  - "R3.2: 不允许修改 REQUIREMENT.md / DESIGN.md"
  - "R7.1: 不允许超出 TASK.md write_files 范围"
...
- 开发执行（4-dev）：按 TASK.md 中的单个任务执行 TDD 开发
- 快速修复（F-fast）：<50 行的 bugfix / 一次性脚本

## 入场协议

1. 读取分配的 task XML 块（id, read_files, write_files, verify）
...
- dev → 按 `phases/4-dev.md` 完整流程
  - TDD：先写测试 → 实现 → 通过
  - 每个 task 在 fresh context 中执行
  - 中断时写 PROGRESS.md（R1.5 恢复协议）
- fast → 按 `phases/F-fast.md`
  - 轻量闭环：直接修复 + verify

## 退出协议

1. verify 命令通过
2. 写 <task-id>-SUMMARY.md
3. git commit（原子提交，R4.1 格式）
4. 更新 TASK.md 对应 task status="done"
