...
```bash
gantry change "描述需求"
/gantry-exec  # 确认 TASKS.md 后在 IDE skill 中执行
gantry archive
```
...
| `/gantry-context evolve` | `phases/A-evolve.md` | 架构增量同步：扫近期归档 change 的 DESIGN §9，逐项 review 后 patch CONTEXT / ARCHITECTURE | 每月 / 每季批量同步；里程碑发布后凝固架构决策 |
...
| `templates/SPEC.md` | 需求 + 验收准则（Given/When/Then）+ v1·v2·out | ✅ 独立 | 任何项目的需求文档模板 |
...
每个 change 完成 → DESIGN.md §9 填「新增抽象」「项目级决策」
             ↓ N 个 change 后
/gantry-context evolve → 扫各 change 的 §9 → patch CONTEXT + ARCHITECTURE
...
| 只想写好一份需求文档 | `phases/1-requirement.md` + `templates/SPEC.md` + `templates/CONTEXT.md` | 其他都不要 |
