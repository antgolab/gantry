...
用户意图
    ↓
┌──────────────────────────────────────────────┐
│  IDE 斜杠命令层（/gantry-change 等）           │  ← 日常主路径，零配置，gantry install 后即用
│  CLI 状态机层（gantry install / change / archive）│  ← 安装、建 change、归档等机械操作
│  @引用层（docs/GO.md 等）              │  ← 通用回退，任何支持 @ 的 AI 工具
└──────────────────────────────────────────────┘
    ↓
阶段执行（phases/*.md）                          ← 执行内容，与工具无关
    ↓
.gantry/specs/<change-id>/*.md                          ← 制品，版本化，可追溯
```
...
├── PROPOSAL.md      ← 变更动机、影响面评估、范围排除
├── SPEC.md          ← 验收准则（Given/When/Then）、用户故事、v1/v2/out
├── DESIGN.md        ← 技术决策、ADR、风险矩阵、§9 架构沉淀建议
├── TASKS.md         ← 原子任务、并行波次、verify 命令、依赖关系
├── EXECUTION.md     ← change 级执行记录：做了什么、verify 输出、决策偏离
├── T01-SUMMARY.md   ← 仅高风险/例外任务的专用完成报告
...
| **知识库 + 月度维护** | LESSONS.md UAT 失败提名，`/gantry-knowledge curate` 月度整理 + 过期检测 | 知识流失：踩坑经验无法复用 |
| **约定漂移检测** | CONVENTIONS 模板 + 季度 diff 检查实际代码 | 知识流失：团队规范与实现偏离 |
| **入场扫描护栏** | `/gantry-context scan` 生成 CONTEXT.md，DESIGN 强制对齐既有架构 | 范围侵蚀：新代码破坏既有抽象 |
...
```bash
cd your-project
gantry install --tool claude   # 可选: claude / codex / cursor / copilot / all
```

`gantry install` 自动完成：
- 创建 `.gantry/planning/STATE.md`（状态机）
- 创建 `.gantry/planning/config.json`（管线配置）
- 复制 `.gantry/core/phases/*.md`（阶段协议）
- 把公开 skills 注入 IDE 对应目录（Claude/Codex 用 `skills` 目录，Cursor/Copilot 用规则 / prompt 文件）
- 在 `CLAUDE.md` / `AGENTS.md` 写入 Gantry 规则块

后续再次运行 `gantry install` 会同步 / 重装客户端命令集成，并保留已有 `.gantry/planning/` 状态。
...
| 自动化 | `/gantry-auto` | 在 人工确认关卡下自主推进 |
| 审查 | `/gantry-review`、`/gantry-review --requirement`、`/gantry-review --adversarial` | 代码审查、需求门禁、对抗性审查 |
| 项目上下文 | `/gantry-context scan`、`/gantry-context architect`、`/gantry-context evolve` | 入场扫描、架构梳理、架构沉淀同步 |
| 知识库 | `/gantry-knowledge capture`、`/gantry-knowledge curate` | 捕获知识、月度 / 季度维护 |
| 诊断与快修 | `/gantry-health`、`/gantry-debug`、`/gantry-fast` | 代码库巡检、系统化调试、快速路径 |

> `scan` / `architect` / `evolve` / `curator` / `check-req` / `doubt` 不再作为独立公开命令暴露；对应能力已收敛到 `context`、`knowledge`、`review` 三个入口下。
...
```bash
gantry change "描述需求"   # → PROPOSAL.md
/gantry-next               # → SPEC.md
/gantry-next               # → DESIGN.md
/gantry-next               # → TASKS.md
/gantry-exec              # → DEV（在 IDE skill 中逐任务循环）
/gantry-next               # → TEST
/gantry-next               # → REVIEW
...
只有文档、配置、文案或局部 bug 等低风险变更可显式选择 light：

```bash
gantry change --pipeline light "描述需求"
/gantry-next  # Change 确认后进入 Fast
gantry archive
```

### MVP 模式（light 管线，当天跑通）

light 只走 `change → fast → integration`；Fast 内合并范围锁定、实现、测试和自审：
...
产物：`PROPOSAL.md` + `EXECUTION.md`。命中高风险边界会拒绝 light，需改走 full。
...
| `phases/1-requirement.md` | 把变更提案变成可执行需求 + 验收准则 + CONTEXT 候选 patch | ✅ 完全独立 | 写需求文档；把口头需求变成 PRD；提取但不直接固化项目上下文 |
...
| `phases/4-dev.md` | 在 fresh context 中执行**单个任务**（含 TDD + 提交 + 断点恢复 + UI anti-pattern 扫描）| ⚠️ 半独立 | 严格依赖 `TASKS.md`（兼容旧 `TASK.md`）中的 `verify` 字段 |
...
横向能力仍由 `phases/*.md` 承载，但不再把每个内部 phase 暴露成一个独立命令。日常只记 4 个入口：

| 公开入口 | 内部 phase | 作用 | 单独用场景 |
|---|---|---|---|
| `/gantry-context scan` | `phases/I-intel-scan.md` | 老项目入场扫描：检测 AI 上下文文档 + 扫代码，生成 / 更新 `CONTEXT.md` | Brownfield 项目首次使用 Gantry；老项目架构偏移后重扫 |
| `/gantry-context architect` | `phases/A-architect.md` | 项目级架构梳理：建立 / 重构 `ARCHITECTURE.md`，含模块图 + ADR 列表 + 跨模块契约 | 里程碑后架构梳理；接手陌生项目；ADR 定期重审 |
| `/gantry-context evolve` | `phases/A-evolve.md` | 上下文增量同步：扫近期归档 change 的 SPEC 候选 patch + DESIGN §9，逐项 review 后 patch CONTEXT / ARCHITECTURE | 每月 / 每季批量同步；里程碑发布后凝固架构决策 |
| `/gantry-knowledge curate` | `phases/C-curator.md` | 知识库维护 + 团队健康巡检：LESSONS 状态整理 / knowledge 过期检测 / CONVENTIONS 漂移检测 | 月度知识库维护；季度规则 retro；事故后知识沉淀 |
| `/gantry-health` | `phases/M-health.md` | 代码库周期性巡检：6+6 维衰退诊断 / 技术债优先级 + 冗余扫描 | 月度 / 季度体检；里程碑前估价；接手陌生项目首周 |
| 手动引用 `phases/L-restyle.md` | `phases/L-restyle.md` | 一键换调性：保留功能不变，只换视觉 | 产品视觉陈旧；品牌换新；做暗色版 / 高端版 |

### 制品模板（14 个）

| 文件 | 作用 | 能否单独用 | 单独用场景 |
|---|---|---|---|
| `templates/PROPOSAL.md` | 变更提案 | ✅ 独立 | 任何项目的「变更申请」「功能提案」 |
| `templates/SPEC.md` | 需求 + 验收准则（Given/When/Then）+ v1·v2·out + CONTEXT 候选 patch | ✅ 独立 | 任何项目的需求文档模板 |
| `templates/CONTEXT.md` | 项目级共享上下文 · **rules 层**（技术栈 + 术语表 + 已锁决策 + 既有抽象索引 + 禁动清单）| ✅ 独立，**强烈推荐** | 通过 `/gantry-context scan` 首创；不要用单个 change 凑薄 CONTEXT |
...
| `templates/TASKS.md` | 任务清单（XML + 波次 + verify + done） | ✅ 独立 | 任何工作分解；可直接当 todo 看板用 |
| `templates/TEST.md` | 测试矩阵 + UAT 脚本 + 覆盖率回顾 | ✅ 独立 | 任何测试计划 / QA 文档 |
| `templates/REVIEW.md` | 双轮审查报告（spec 合规 + 代码质量 + 跨模型分歧）| ✅ 独立 | 任何 code review 报告模板 |
| `templates/EXECUTION.md` | change 级执行记录（做了什么 + verify 输出 + 决策偏离）| ✅ 独立 | 执行账本 / 任务汇报 / 闭环证据 |
| `templates/SUMMARY.md` | 高风险/例外任务完成报告（做了什么 + verify 输出 + 决策偏离）| ✅ 独立 | 例外任务回执 / 审计留痕 |
| `templates/LESSONS.md` | **项目级常驻**——结构化跨任务失败知识库 | ✅ 独立，**强烈推荐** | 任何项目；踩坑后按完整字段追加或先放待审 |
...
`.gantry/specs/` 下有三层文档，职责不重叠：

| 文件 | 职责 | 维护者 | 体量 |
|---|---|---|---|
| `.gantry/specs/CONTEXT.md` | **rules 层**：技术栈版本 / 命名约定 / 既有抽象索引 / 禁动清单 | `/gantry-context scan` 首创 + `/gantry-context evolve` 增量 | 50–200 行 |
| `.gantry/specs/ARCHITECTURE.md` | **structure 层**：模块图 / ADR 列表 / 跨模块契约 / 容量边界 | `/gantry-context architect` 首创 + `/gantry-context evolve` 增量 append | 200–600 行 |
| `.gantry/specs/<change-id>/DESIGN.md` | **change 层**：本次 change 的技术决策 / ADR / 风险 / §9 沉淀建议 | `2-design` 写，归档后冻结 | 100–400 行 |

联动方式：

```
每个 change 完成 → SPEC.md「CONTEXT 候选 patch」+ DESIGN.md §9 填「新增抽象」「项目级决策」
             ↓ N 个 change 后
/gantry-context evolve → 扫各 change 的候选 patch + §9 → 逐项 review 后 patch CONTEXT + ARCHITECTURE
             ↓ ADR 冲突 ≥ 5 或 CONTEXT > 200 行
/gantry-context architect → 重审并整理 ARCHITECTURE.md
```
...
| 只想理顺一个想法 | `phases/0-change.md` + `templates/PROPOSAL.md` | 其他都不要 |
| 只想写好一份需求文档 | `phases/1-requirement.md` + `templates/SPEC.md` | `CONTEXT.md` 不作为单次需求文档模板；只产候选 patch |
| 只想做技术设计 | `phases/2-design.md` + `templates/DESIGN.md` + `reference/tech-stacks.md` | 其他都不要 |
| 只想拆好一组任务 | `phases/3-task.md` + `templates/TASKS.md` | 其他都不要 |
...
| 只想做代码库体检 | `/gantry-health`（内部读取 `phases/M-health.md`）| 其他都不要 |
| 只想整理知识库 | `/gantry-knowledge curate`（内部读取 `phases/C-curator.md`）| 其他都不要 |
...
| 给 AI 一个项目级"小抄" | `/gantry-context scan` 生成 `CONTEXT.md`，后续用 `/gantry-context evolve` 增量同步 | 不要用单个 change 的术语/决策直接凑薄 CONTEXT |
| 想跑完整闭环 | 全套 | — |
...
| 单独跑 | 典型 token | 产出 |
|---|---|---|
| `0-change.md` | ~6k – 8k | PROPOSAL.md |
| `1-requirement.md` | ~6k – 9k | SPEC.md（AC 完备）|
| `2-design.md` | ~10k – 15k | DESIGN.md（含技术栈卡片）|
| `2a-ui-design.md` | ~12k – 18k | UI-DESIGN.md（含调性 + tokens）|
| `3-task.md` | ~8k – 12k | TASKS.md（N 个任务）|
| `4-dev.md` × 单个 task | ~25k – 60k | 实现代码 + 测试 + EXECUTION（必要时补 SUMMARY） |
...
| 护栏 | 文件 | 拦截的事故类型 |
|---|---|---|
| 入场扫描自动生成 CONTEXT.md | `/gantry-context scan` | AI 不知道项目栈 / 命名 / 既有抽象 → 写出格格不入的代码 |
...
```bash
/gantry-context scan       # 生成 CONTEXT.md（仅首次，+15k–30k tokens）
gantry change "加功能"     # 之后可用 CLI 或 IDE skill 正常走流程
```
...
3. **任务必带 verify 命令**：`TASKS.md`（兼容旧 `TASK.md`）每项含可执行验证命令，否则不允许进入执行
...
- [x] LESSONS.md 知识库 + `/gantry-knowledge curate` 月度维护
...
- **GitHub Actions 集成**：PR 合并前自动检查 `.gantry/specs/<change-id>/` 制品完整性
...
- 工程师默认走 full；只有明确选择时才进入受限 light 路径
