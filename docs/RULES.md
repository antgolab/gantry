...
- **R1.9 · 工件加载预算（任何阶段首轮强制）**
  ...
  5. 完整加载策略表见 `docs/GO.md` 顶部「红线·Token 预算」段
...
- **R4.6 · 破坏性变更高门槛（强制）**
  ...
  AI 在 4-dev 阶段命中下面**任一**条件时，禁止直接动手，必须先走「破坏性变更协议」（详见 `phases/4-dev.md` 第 1.8 节）：
...
### UI 视觉扩展（仅 2a-ui-design 阶段生效）

复用 R8 前缀便于 grep。详见 `@phases/2a-ui-design.md`。

- **R8.3** 颜色必须 OKLCH（除非有遗留约束并显式声明）
- **R8.4** 字体必须避开 AI slop 默认词
- **R8.5** 每个美学决策必须给理由
- **R8.6** UI-DESIGN.md 输出前必须 grep `ui-anti-patterns.md` 完成自检
- **R8.7** brownfield 必走 1.5，视觉语汇对齐报告必须经用户确认
- **R8.8** v0 草稿必须在写完整 UI-DESIGN.md 前给用户确认
- **R8.9** 禁止编造数据 / 好评 / logo；缺素材走占位符策略
