...
- "消费者"可以是代码，也可以是人；但必须是**当前真实存在**的，不是设想中的。
...
* 仅本次 Proposal 声明 `uiImpact: true` 时走 2a
```

**full（默认）**：`PROPOSAL → SPEC → DESIGN → [UI-DESIGN] → TASKS → DEV → TEST → REVIEW → INTEGRATION`
**light（仅显式低风险）**：`PROPOSAL → FAST → INTEGRATION`，Fast 内完成实现、验证和自审，只产 `PROPOSAL.md` + `EXECUTION.md`
...
| REQUIREMENT | PROPOSAL | 写 AC + 范围切分 + 术语提取 + CONTEXT 候选更新 | `SPEC.md` + `CONTEXT.md` 候选 patch（仅通过质量门才落库） | 否 | 否 |
| DESIGN | PROPOSAL + SPEC + CONTEXT | **技术栈预选**（5~6 卡让用户选）+ 技术决策 + ADR + 数据流 | `DESIGN.md`（含 `## 0. 技术栈选定`）| 是（栈 + 关键决策）| 条件性 |
| **2a UI-DESIGN** | SPEC + DESIGN | 美学方向 + design tokens + 反 AI-slop 自检 | `UI-DESIGN.md` | 否 | `uiImpact=false` 时跳过 |
...
| REVIEW | diff + SPEC | 双轮审查（spec 合规 + 代码质量）| `REVIEW.md` | 否 | 否 |
| FAST | PROPOSAL | 低风险实现 + 测试 + 自审 | 代码 + `EXECUTION.md` | 否 | 仅 light |
| INTEGRATION | 全部已通过 | UAT + 集成 smoke + 失败诊断 | `UAT.md` / fix-plan | 是 | 否 |
| ARCHIVE | 已收尾 | 保存 change 归档工件 | `_archive/<change-id>/` | 否 | — |

> `archive` 完成当前 change 收尾并把 `.gantry/specs/<id>/` 移动归档到 `.gantry/specs/_archive/<id>/`（源目录删除），然后把 STATE 重置为 idle。`/gantry:unarchive <change-id>` 是反向生命周期动作：从归档恢复工件并重新激活该 change。开发 / 测试中发现变化时，用 `/gantry:adjust "<发生了什么>"` 打开或追加当前 change 的 `PATCH.md` 闭环账本。
...
| `CONTEXT.md` | **项目级 rules 层**：技术栈 / 域语言 / 已锁决策 / 既有抽象索引 / 禁动清单 / 技术债。它回答"AI 写代码时必须遵守什么"，不是单个 change 的备忘录 | `I-intel-scan` 首创；`A-evolve` / 人工确认后增量同步 |
...
| `STATE.md`（`.gantry/planning/STATE.md`）| 跨会话状态（当前位置 / 中断任务 / 暂停原因 / 高价值决策）| AI 维护，人可改 |

> 兼容迁移期内，运行时仍接受旧命名：`CHANGE.md` / `REQUIREMENT.md` / `TASK.md` / `SUMMARY.md`。新产出应优先使用新命名。

### CONTEXT 写入边界

`CONTEXT.md` 只收**后续 change 会直接消费的项目级规则**。单个 change 内才有意义的事实，写在该 change 的 `SPEC.md` / `DESIGN.md` / `EXECUTION.md`，不要升入 CONTEXT。

| 信息类型 | 默认位置 | 进入 CONTEXT 的条件 |
|---|---|---|
| 本次业务细节 / 临时约定 | `<change-id>/SPEC.md` | 明确会影响后续 ≥ 2 个 change，且有来源文件 / 行号 |
| 项目术语 | `CONTEXT.md`「域语言」 | 后续需求 / 代码里会反复使用该词 |
| 已锁技术 / 业务规则 | `CONTEXT.md`「已锁决策」 | 已被用户确认，且与 LESSONS / 现有 CONTEXT 无冲突 |
| 既有抽象 / 禁动清单 / 技术债 | `CONTEXT.md` 对应段 | 只能由 `I-intel-scan`、`M-health`、`A-evolve` 或人工确认 patch 写入 |
| 失败教训 / 被否决方案 | `LESSONS.md` | 满足提名条件，且按 LESSONS 条目模板填齐 |

**薄 CONTEXT 禁止落库**：如果一个新项目只有术语 / 决策，没有技术栈、既有抽象索引、命名约定、禁动清单、intel-scan 元数据，它不是合格的 `CONTEXT.md`，只能作为 SPEC / DESIGN 的候选补丁，必须先跑 `/gantry-context scan` 或人工补齐。
