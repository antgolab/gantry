...
> **目的**：验证 4-dev / 5-test / 6-review 的「装了 brooks-lint」路径 A 与「内置回退」路径 B 都能产出 **4 要素 + R/T 编号** 的统一格式，让 Gantry 下游（fix 任务追加、TASK.md 编号延续）无缝接收。
>
...
phases/4-dev.md
@gantry/examples/brooks-lint-paths.md
...
### 你是用户想试用 Gantry + brooks-lint

```
docs/GO.md
帮我用这个 demo（@gantry/examples/brooks-lint-paths.md）走一遇 4-dev 的 self-review
```

GO.md 路由命中 `执行 T<NN>` / `跑` 类触发词 → `phases/4-dev.md` → AI 进入步骤 4，按你装没装 brooks-lint 自动选路径 A/B。
...
**只要两条路径都过「格式一致性」+「下游可接收」段，融合就成功了**。「路径差异」段的 gap 是 brooks-lint 本身的价值（带书本引用 + 系统化覆盖），不是 Gantry 集成的问题。
