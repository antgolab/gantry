...
> 项目级常驻文件，位置：`.gantry/specs/LESSONS.md`
...
新增 / 修改 active 条目前必须逐项确认：

- [ ] 字段齐全：`首发`、`上次复核`、`适用栈`、`状态`、`关键词`、五个正文小节都存在
- [ ] 关键词能被后续任务 grep 命中，至少包含 2 个代码符号 / 文件名 / API 名 / 业务名
- [ ] `为什么不行` 含可复现信号：错误输出、失败测试、度量、代码路径或明确条件
- [ ] `当前推荐做法` 是可执行规则，不只是"注意检查"
- [ ] 已 grep `CONTEXT.md`，若与已锁决策冲突，先同步修正 CONTEXT 或把本条放入 `## 待审`
- [ ] 与现有 active LESSONS 不重复；若重复，旧条目标 `superseded-by:L-NNN`

缺任一项时，不得直接放入 active 条目区；先写入 `## 待审`，等待 `/gantry-knowledge curate` 合入。

---

## 待审

> INTEGRATION 阶段可把证据不足或字段未补齐的候选放这里。Curator 月度整理时决定合入 / 丢弃 / 合并。

<!--
### PENDING · <一句话标题>
- **来源**: <change-id> · <task-id> · <YYYY-MM-DD>
- **缺口**: <缺哪些字段 / 证据>
- **候选内容**: <简述>
-->
...
- **季度剪枝**：deprecated > 6 个月的条目移到 `.gantry/specs/archive/LESSONS-archive.md`
...
- 详见 `.gantry/specs/archive/2026-04-30-add-dark-mode/SUMMARY.md` 的"决策与偏离"段
